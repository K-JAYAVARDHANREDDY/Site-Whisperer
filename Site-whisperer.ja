    ...
    tooltip.innerHTML = `
      <div class="sw-tooltip__inner">
        ...
        <div class="sw-tooltip__footer">
          ...
          <span class="sw-tooltip__hint">Powered by Claude AI</span>
        </div>
      </div>
    `;
...
What is this section for? Explain to a regular user.`;
  ...
  const SiteWhisperer = {
    init(userConfig) {
      if (!userConfig.apiKey) {
        console.error("[SiteWhisperer] apiKey is required.");
        return;
      }
  ...
  function _mount() {
    ...
    setupMouseListeners();
    console.log(
      `%c✦ SiteWhisperer ready%c  Hold ${config.triggerKey} or click the badge to activate AI insights.`,
